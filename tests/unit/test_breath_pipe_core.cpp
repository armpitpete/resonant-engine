...
void testExcitationExternalAndInteraction() {
    ...
    (void)events.push({0, resonant::EventType::Pitch, 0, 0, 220.0F, 0.0F});
    (void)events.push({0, resonant::EventType::Pressure, 0, 0, 0.42F, 0.0F});
    ...
    for (std::uint32_t block_index = 0; block_index < 240U; ++block_index) {
        ...
        check(engine.process(block, scheduled) == resonant::ProcessStatus::Ok,
              "pressure/turbulence render remains finite");
        if (block_index > 80U) {
    ...
    (void)external_events.push({0, resonant::EventType::Pitch, 0, 0, 220.0F, 0.0F});
    (void)external_events.push({0, resonant::EventType::ParameterChange,
                                resonant::BreathPipeVoice::kExternalAmount, 0, 1.0F, 0.0F});
    ...
    for (std::uint32_t block_index = 0; block_index < 160U; ++block_index) {
        for (std::size_t frame = 0; frame < input.size(); ++frame) {
            const auto sample_index = static_cast<double>(block_index * 128U +
                                                          static_cast<std::uint32_t>(frame));
            input[frame] = static_cast<float>(0.18 * std::sin(
                6.283185307179586 * 173.0 * sample_index / 48'000.0));
        }
    ...
    auto interactionSignature = [](float interaction) {
    ...
    check(std::abs(interactionSignature(0.0F) - interactionSignature(1.0F)) > 1.0e-4,
...
void testOverblowAndRegeneration() {
    ...
    auto collectModeEnergy = [&resonator](const auto& control, std::uint32_t period) {
        std::array<double, 3> energy{};
        resonator.reset();
        for (std::uint32_t frame = 0; frame < 48'000U; ++frame) {
            const auto centred = static_cast<int>(frame % period) -
                                 static_cast<int>(period / 2U);
            const auto excitation = frame == 0U
                                        ? 0.00038F
                                        : 0.000002F * static_cast<float>(centred);
    ...
    const auto low_energy = collectModeEnergy(low, 19U);
    const auto low_overblow = resonator.overblowAmount();
    const auto high_energy = collectModeEnergy(high, 23U);
    ...
    for (std::uint32_t frame = 0; frame < 96'000U; ++frame) {
        ...
        const auto result = resonator.processSample(excitation, self_sustain);
        if (frame >= 72'000U) {
            sustain_tail += std::abs(static_cast<double>(result.sample));
        }
    }
    ...
    for (std::uint32_t frame = 0; frame < 48'000U; ++frame) {
        const auto result = resonator.processSample(0.0F, recovery);
        if (frame >= 36'000U) {
            recovery_tail += std::abs(static_cast<double>(result.sample));
        }
    }
...
void testPitchAndExtremeBehaviour() {
...
int main() {
    ...
    testExcitationExternalAndInteraction();
    testOverblowAndRegeneration();
    testPitchAndExtremeBehaviour();
    if (failures != 0) {
        std::cerr << failures << " Breath Pipe core test(s) failed\n";
        return 1;
    }
    std::cout << "PASS: M3 Breath Pipe core operating envelope and behaviour\n";
    return 0;
}
